...
//This line initiates a parallel for loop using OpenMP. It instructs the compiler to parallelize the loop. Each iteration of the loop will be executed in parallel by different threads. The reduction clause specifies that the reduction operation for the max_value variable should be performed after all iterations are complete. Specifically, it means that each thread will maintain its own max_value, and at the end of the parallel section, OpenMP will compute the maximum of all thread-local max_value variables and store it in max_value.
...
chrono::duration<double, micro> sqMin = endSeqMin - startSeqMin;
...
chrono::duration<double, micro> sqMax = endSeqMax - startSeqMax;
...
chrono::duration<double, micro> sqSum = endSeqSum - startSeqSum;
...
chrono::duration<double, micro> sqAvg = endSeqAvg - startSeqAvg;
